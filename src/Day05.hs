...
solvePart1 (Problem definition) =
    ...
    where Definition {orders = ords, manuals = mans} = definition
...
reorderManual m ords =
    let
        relevantOrds = filter (\(l,r) -> l `elem` m && r `elem` m) ords
        new_ord = foldl orderPages m relevantOrds
    in
        if new_ord == m then m else reorderManual new_ord ords
    where
...
parseInput input =
    ...
    in
        Problem $ Definition {orders = _orders, manuals = _manuals}
    where
        parseOrder :: String -> OrdRule
        parseOrder o =
            o |> map (\x -> if x =='|' then ' ' else x) |>
            words |> map read |> (\x -> (head x, x!!1))--(\(x1:x2:_) -> (x1, x2))
        parseManual :: String -> Manual
        parseManual m =
            m |> map (\x -> if x == ',' then ' ' else x) |>
            words |> map read
