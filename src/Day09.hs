...
cleanBlock idx ((B2 blen bidx):blocks) =
    if idx == bidx then E2 blen:blocks
...
reorder blocks =
    ...
    where
        ...
        sort_iterate (b:bs) (rb:rbs) i ri =
            if i == ri then b : replicate (len - ri) E
            else case (b, rb) of
...
parsePart1 input =
    ...
    where
        ...
        block idx =
            ...
            -- all even positions are empty space
            in if m == 0 then B b else E
